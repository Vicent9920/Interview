...
public class MainPresenter implements MainContract.Presenter{
    ...
    private List<InterViewInfo> data = new ArrayList<>();
    ...
    public void start() {
        ...
        if(data.size()>0)
            data.clear();
        final List<InterViewInfo> readInterViewInfo = new ArrayList<>();
        BmobQuery<InterViewInfo> queryRead = new BmobQuery<InterViewInfo>();

        if(user.getReaded().size() == 0){
            mView.showErrorInfo("目前还没有数据呢!");
        }else{
            queryRead.addWhereContainedIn("objectId", user.getReaded());
            queryRead.setLimit(500);
            queryRead.findObjects(new FindListener<InterViewInfo>() {

                @Override
                public void done(List<InterViewInfo> object,BmobException e) {
                    if(e==null){
                        readInterViewInfo.addAll(object);
                        if(isRead){
                            if(readInterViewInfo.size()==0){
                                mView.showErrorInfo("目前还没有数据呢!");
                            }else{
                                data.addAll(readInterViewInfo);
                                mView.showResult();
                            }
                        }

                    }else{
                        if(NetUtil.isConnected(mView.getContext())){
                            mView.showErrorInfo(mView.getContext().getString(R.string.main_loaderror));

                        }else{
                            mView.showErrorInfo("请打开网络连接!");
                        }
        ...
        if(!isRead){
            ...
            query.findObjects(new FindListener<InterViewInfo>() {
                @Override
                public void done(List<InterViewInfo> resultData, BmobException e) {
                    if(e==null){
                        LUtil.e("后台数据："+resultData.size());
                        if(resultData.size()==0){
                            mView.showErrorInfo("目前还没有数据呢!");
                        }else{
                            data.addAll(resultData);
                            if(data.size() != readInterViewInfo.size() && readInterViewInfo.size()>0){
                                delReadInterView(readInterViewInfo);
                            }
                            mView.showResult();
                        }
...
//        }
    ...
    /**
     * 删除已读资源
     * @param readInterViewInfo
     */
    private void delReadInterView(List<InterViewInfo> readInterViewInfo) {
        for (int i = 0; i < readInterViewInfo.size(); i++) {
            for (int j = 0; j < data.size(); j++) {
                if(readInterViewInfo.get(i).getObjectId().equals(data.get(j).getObjectId())){
                    data.remove(j);
                    break;
                }
            }
...
//                                }
    ...
    public List<InterViewInfo> getData(){
...
//查询playerName叫“比目”的数据
        query.addWhereEqualTo("part", part);
...
//执行查询方法
     ...
     * @return
     */
    private int getIndex(InterViewInfo bean) {
